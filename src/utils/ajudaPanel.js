...
const COMANDOS = [
 ...
 { grupo: 'Compras', cmd: 'encomenda', desc: 'Solicita um item fora do estoque imediato (encomenda).', admin: false },
 ...
 { grupo: 'Painéis', cmd: 'painelcategoria', desc: 'Sem arg: seletor visual de categoria. Escolhe o canal para publicar.', admin: true },
 ...
 { grupo: 'Administração', cmd: 'setcomprar', desc: 'Define a mensagem de pagamento e a chave PIX do /comprar ({total}, {pix}, {canal}).', admin: true },
 { grupo: 'Administração', cmd: 'autoresposta', desc: 'Gerencia respostas automáticas por palavra (adicionar, ver, editar, remover, canais).', admin: true },
 { grupo: 'Administração', cmd: 'canalcomando', desc: 'Configura em quais canais cada comando pode ser usado.', admin: true },
...
const PAGINAS = {
  ...
  compras: () => ({
    ...
    descricao: [
      ...
      '',
      '➜ **/comprar**',
      '*Inicia a compra: escolha categoria, produto e quantidade. Cada quantidade escolhida ja entra no carrinho — use **Finalizar compra** quando terminar.*',
      '**O carrinho e o pedido aparecem no ticket**, visiveis para o cliente e para a equipe.',
      '',
      '`🛒 Ver carrinho` mostra os itens, o total e o menu para escolher um produto.',
      'Ao escolher um produto você pode `➕ Aumentar`, `➖ Diminuir` ou `🗑️ Remover` — para retirar por completo, use Remover.',
      'Os controles do carrinho são: `🎁 Escolher mais produtos` · `🧹 Limpar carrinho` · `✅ Finalizar compra`.',
      '',
      '➜ **Confirmar / cancelar pedido (equipe)**',
      '*Depois de finalizar, o pedido fica `🟡 Aguardando pagamento` no ticket. Quem tem o grupo **🛒 Vendas e pedidos** usa `🟢 Confirmar pagamento` (vira `🟢 Pago` e baixa o estoque) ou `❌ Cancelar pedido` (libera a reserva).*',
      '',
      '➜ **/cliente <@usuário>**',
      '*Mostra os cargos de cliente já conquistados.*',
      '',
      '➜ **/encomenda**',
      '*Solicita um item que não está no estoque imediato. Escolha categoria, item e quantidade, confira o resumo e confirme.*',
      '**A encomenda não reserva nem baixa estoque** — é um processo separado da compra.',
      '`💳 Entrada` de 50% paga antes do início e `💵 Restante` de 50% antes da entrega.',
      'A equipe confere o comprovante e depois avança a etapa — o bot não marca pagamento automaticamente.',
      '`🟣 Aguardando início` → `🟠 Em andamento` → `🔵 Item recebido` → `🟢 Entregue`.',
      'A equipe (grupo **🛒 Vendas e pedidos**) usa os botões do próprio ticket para avançar ou cancelar.',
      '',
      '➜ **/metas (adm)**',
      '*Configura metas de cargos para clientes.*',
      '',
      '➜ **/setcomprar (adm)**',
      '*Define a mensagem de pagamento e a chave PIX usadas no pedido. Variáveis: `{total}`, `{pix}` e `{canal}`.*',
      '',
      '➜ **/logcompras (adm)**',
      '*Configura o canal donde ficam os logs das compras e encomendas (pedido criado/pago/cancelado e as etapas da encomenda).*',
      '',
      '➜ **/proof (adm)**',
  ...
  painel: () => ({
    ...
    descricao: [
      ...
      '',
      '➜ **!painelcategoria <id> (adm)** — *Fixar a categoria: sem id abre o seletor; com <id> fixa direto. No seletor, escolhe em qual canal publicar.*',
      '',
  ...
  admin: () => ({
    ...
    descricao: [
      ...
      '',
      '➜ **/comprar (adm)**',
      '*Aprova ou cancela pedidos: confirme o pagamento ou cancele pelos botões `🟢 Confirmar pagamento` / `❌ Cancelar pedido` no proprio pedido do ticket.*',
      '',
      '➜ **/setcomprar (adm)**',
      '*Define a mensagem de pagamento (`{total}`, `{pix}`, `{canal}`) e a chave PIX mostrada no pedido.*',
      '',
      '➜ **/logcompras (adm)**',
      '*Define o canal onde ficam os logs das compras e encomendas (pedido criado/pago/cancelado, etapas da encomenda e metas).*',
      '',
      '➜ **!embed (adm)**',
      ...
      '*Gerencia respostas automáticas por palavra (ver, adicionar, editar, remover, canais).*',
      '',
      '➜ **!canalcomando / /canalcomando (adm)**',
      '*Abre o painel para configurar em quais canais cada comando pode ser usado.*',
      '',
      '➜ **/setproof (adm)**',
      '*Define o canal para onde os comprovantes (proofs) são enviados.*',
      '',
      '➜ **!limpar <1-100> (adm)**',

  ...
  client.on('messageCreate', async (msg) => {
    try {
      ...
      if (cmd === 'delproduto') {
        ...
        await registrarLog(client, guildId, 'produto', msg.author.id, {
          descricao: `<@${msg.author.id}> removeu o produto **${produto.nome}**.`
        }, configs);
      ...
      if (sessoesCriacao.has(sessaoTab)) {
        ...
        if (linhas.length) {
          ...
          const _tab = (() => {
            try { return JSON.parse(_prod?.tabela || '[]'); }
            catch { return []; }
          })();

          _tab.push(...linhas);

          getDB()?.prepare('UPDATE produtos SET tabela = ? WHERE id = ?')
            .run(JSON.stringify(_tab), s.produtoId);
      ...
      if (sessoesCriacao.has(sessaoKey)) {
        ...
        const etapas = {
            ...
            await msg.reply({
              embeds: [
                new EmbedBuilder()
                  .setColor(0xf39c12)
                  .setTitle('➕ Passo 3/5')
                  .setDescription('**Descrição** do produto (ou `pular` para deixar em branco):')
              ]
            });
          },

          descricao: async () => {
            s.dados.descricao = msg.content.toLowerCase() === 'pular'
              ? ''
            ...
            await msg.reply({
              embeds: [
                new EmbedBuilder()
                  .setColor(0xf39c12)
                  .setTitle('➕ Passo 4/5')
                  .setDescription('URL da **imagem** do produto (ou `pular`):')
              ]
            });
          },

          imagem: async () => {
            s.dados.imagem = msg.content.toLowerCase() === 'pular'
              ? null
              : msg.content.trim();
            ...
            await msg.reply({
              embeds: [
                new EmbedBuilder()
                  .setColor(0xf39c12)
                  .setTitle('➕ Passo 5/5')
                  .setDescription('**Status** do produto:\n`disponivel` | `poucas` | `fechado`')
              ]
            });
          },
          ...
          status: async () => {
            ...
            s.dados.status = statusValidos.includes(msg.content.toLowerCase())
            ...
            await registrarLog(client, guildId, 'produto', msg.author.id, {
              descricao: `<@${msg.author.id}> adicionou o produto **${produto.nome}**.`
            }, configs);
            ...
            await msg.reply({
              embeds: [
                embedSucesso(
                  'Produto Adicionado!',
                  `**${produto.nome}** foi adicionado ao catálogo. Use \`!addtabela ${produto.nome}\` para adicionar preços.`
                )
              ]
            });
          },

const recentesMsgs = new Map();
  ...
  export function register(client, configs) {
    if (client.__antiAbusoRegistrado) return;
    client.__antiAbusoRegistrado = true;
    ...
    client.on('messageCreate', async (msg) => {
      ...
      if (recentes.length > 5) msg._isSpam = true;
      ...
      if (repetida) msg._isFarm = true;
    });

    // ── Limpeza periódica de memória (a cada 5 min) ─────
    setInterval(() => {
      const agora = Date.now();
      for (const [k, v] of recentesMsgs.entries()) {
        if (!v.some(t => agora - t < 60_000)) recentesMsgs.delete(k);
      }
      for (const [k] of ultimasMensagens.entries()) {
        if (!recentesMsgs.has(k)) ultimasMensagens.delete(k);
      }
    }, 5 * 60_000);
  }
  ...
  export function isSpam(userId, guildId) {
    ...
    const agora = Date.now();
    const lista = recentesMsgs.get(chave) || [];
    return lista.filter(t => agora - t < 5_000).length > 5;
  }
